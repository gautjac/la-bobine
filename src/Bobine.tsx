...
import { WIDTH, HEIGHT, HEAD_FADE_F, TAIL_FADE_F, CUE_FADE_F } from "./lib/timeline";
...
const Band: React.FC<{
  ...
  credit: string;
}> = ({ top, height, cues, font, fontSize, textColor, textAlign, credit }) => {
  const frame = useCurrentFrame();
  const spec = FONTS[font] ?? FONTS[DEFAULT_FONT];
  const pad = 84;
  return (
    <div
      style={{
...
export const Bobine: React.FC<BobineProps> = (p) => {
  ...
  const headFade = interpolate(frame, [0, HEAD_FADE_F], [1, 0], clampOpts);
